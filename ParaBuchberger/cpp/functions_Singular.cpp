...
si_link ssi_open_for_read (std::string const& ssi_filename)
{
  ...
  si_opt_1 = n98_o1; // SET GLOBAL VARIABLES
  si_opt_2 = n98_o2;
...
void ssi_close_and_remove (si_link l)
{
  ...
  if (res)
  {
    throw std::runtime_error ("Fatal error: Failed closing ssi link!");
  }
...
namespace singular {
  ...
  //compromised about the Singular return value over Z_p
  ...
  //notice that the library loaded in the below are somehow the compromised model of the testlist.txt
